...
> **hub は必須ではありません。** デバイスは hub なしでもクラウドストレージに直接同期できます。
> hub を設置すると、Tailscale経由の高速直接同期・ZFSスナップショット・クラウドAPIコスト削減が利用可能になります。
>
...
1. **クラウドストレージのセットアップ** — バケットと S3 API クレデンシャルの作成
