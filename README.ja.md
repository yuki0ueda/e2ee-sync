...
- プライベートなメモ、日記、研究ドラフト
- 確定申告、請求書、事業計画
- 長期暗号化アーカイブ（法令保管文書、過去プロジェクト、個人記録）
...
| パターン | 例 | なぜ e2ee-sync |
|---------|---|--------------|
| デバイス間同期 | ノートPC ↔ デスクトップ | 機密ファイルを常に最新に |
| 暗号化バックアップ | PC → クラウド | クライアント側暗号化でオフサイトバックアップ |
| 長期アーカイブ | 確定申告、契約書 | 安価なクラウド（$0.004/GB）+ 自分で管理する暗号化 |
| クレデンシャル同期 | `.env`、SSH鍵、KeePass DB | クラウド事業者を信頼せずに秘密情報を同期 |
...
e2ee-sync は、他の E2EE サービスが提供する機能（モバイルアプリ、Web UI、ファイル共有、クラウドプレビュー）を意図的に省いています。これは制約ではなく、設計上の選択です。

**利便性より最小攻撃面。** 復号されたデータに触れる機能は全て潜在的な攻撃経路になります。モバイルアプリがあれば紛失・侵害リスクのあるスマホに機密情報が存在します。Web UI があれば暗号鍵がブラウザを通過します。共有リンクがあれば暗号化ファイルに URL でアクセスできてしまいます。クラウドプレビューがあればどこかの誰かがファイルを復号しています。

e2ee-sync は違うアプローチをとります:

| 原則 | 実装 |
|------|------|
| **ファイルはローカルに** | `~/sync` は普通のフォルダ。通常のアプリでファイルを開く。専用ビューアー不要。 |
| **鍵はデバイスから出ない** | 暗号化/復号は自分のマシンでのみ実行。サーバー側処理なし。 |
| **アプリ依存なし** | データは標準的な rclone crypt + 標準的な S3 オブジェクト。e2ee-sync が消えても rclone で直接復号可能。 |
| **アカウント不要** | サインアップなし、ログインなし、ユーザー追跡なし。ストレージは自分で用意。 |
| **オープンソース** | コードは全行監査可能。暗号化は rclone の実績ある crypt 方式。独自暗号ではない。 |

**もしこのプロジェクトが放棄されたら？** データは残ります。標準的な rclone crypt + 標準的な S3 バケット — どちらもオープンで文書化されたフォーマットです。いつでも直接アクセスできます:

```bash
rclone mount cloud-crypt: /mnt/sync   # ローカルドライブとしてマウント
rclone copy cloud-crypt: /local/dir   # 全ファイルをダウンロード
```

独自フォーマットなし。ベンダーロックインなし。特別なツール不要。データは永久にあなたのものです。

復旧手順の詳細（PC 紛失、プロバイダー移行、同期破損、パスワード紛失）は **[障害復旧ガイド](docs/disaster-recovery.ja.md)** を参照。
...
予期しないコストを防ぐには **[コスト管理ガイド](docs/cost-control.ja.md)** を参照 — 課金アラート、転送制限、フィルタルールをカバー。
